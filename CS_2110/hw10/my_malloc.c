...
static metadata_t *find_right(metadata_t *freed_block) {
    ...
    while (curr) {
        if ((uintptr_t) curr == (TOTAL_METADATA_SIZE + freed_block->size + ((uintptr_t) freed_block))) {
            return curr;
        }
        curr = curr->next;
    }
...
static metadata_t *split_block(metadata_t *block, size_t size) {
    size_t totalSize = TOTAL_METADATA_SIZE + size;
    metadata_t *rightBlock = (metadata_t*) (((uintptr_t) block) + block->size + TOTAL_METADATA_SIZE - totalSize);
...
static void add_to_addr_list(metadata_t *block) {
    ...
    metadata_t *previousBlock = NULL;
    metadata_t *currBlock = address_list;
    while (currBlock != NULL && currBlock < block) {
        previousBlock = currBlock;
        currBlock = currBlock->next;
    }
    block->next = previousBlock->next;
...
static metadata_t *find_best_fit(size_t size) {
    ...
    while (currBlock != NULL) {
        if (currBlock->size == size) {
            ...
            if (backUp == NULL || backUp->size > currBlock->size) {
                backUp = currBlock;
            }
        }
        currBlock = currBlock->next;
...
void *my_malloc(size_t size) {
    ...
    metadata_t *freelistBlock = find_best_fit(size);
    if (freelistBlock == NULL) {
        metadata_t *newBlockMEM = my_sbrk(SBRK_SIZE);
        if (newBlockMEM == (void*) -1) {
            my_malloc_errno = OUT_OF_MEMORY;
            return NULL;
        } 
        newBlockMEM->next = NULL;
        newBlockMEM->size = SBRK_SIZE - TOTAL_METADATA_SIZE;
        my_free((void*) (TOTAL_METADATA_SIZE + ((uintptr_t) newBlockMEM)));
        return my_malloc(size);
    } 
    
    if(freelistBlock->size < MIN_BLOCK_SIZE + size) {
        remove_from_addr_list(freelistBlock);
        return ((void*) (TOTAL_METADATA_SIZE + ((uintptr_t) freelistBlock)));
    } 
    
    
    return ((void*) (((TOTAL_METADATA_SIZE + ((uintptr_t) (split_block(freelistBlock, size)))))));
...
void my_free(void *ptr) {
    ...
    metadata_t *freeBlock = ((metadata_t*) ((uintptr_t) ptr - TOTAL_METADATA_SIZE));

    
    metadata_t *afterBlock = find_right(freeBlock);
    metadata_t *beforeBlock = find_left(freeBlock);

    if (afterBlock != NULL) {
        remove_from_addr_list(afterBlock);
        merge(freeBlock, afterBlock);
    }

    if (beforeBlock != NULL) {
        remove_from_addr_list(beforeBlock);
        merge(beforeBlock, freeBlock);
        freeBlock = beforeBlock;
...
void *my_realloc(void *ptr, size_t size) {
    ...
    metadata_t *freeBlock = my_malloc(size);
    size_t prevSize = ((metadata_t*) ((uintptr_t) ptr - TOTAL_METADATA_SIZE))->size;
...
void *my_calloc(size_t nmemb, size_t size) {
    ...
    metadata_t *freeBlock = my_malloc(total);
    if (freeBlock == NULL) {
        return NULL;
    }
    memset(freeBlock, ((char) 0), total);
