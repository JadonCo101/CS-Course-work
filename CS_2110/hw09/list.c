...
static User *create_user(char *name, UserType type, UserUnion data)
{
    ...
    (userPTR->name) = namePTR;
    (userPTR->type) = type;

    if(type == STUDENT) {
        int val = create_student((data.student.num_classes), (data.student.grades), &((userPTR->data).student));
        if (val == 1) {
            free(namePTR);
            free(userPTR);
            return NULL;
        ...
        int val = create_instructor((data.instructor.salary), &((userPTR->data).instructor));
        if (val == 1) {
            free(namePTR);
            free(userPTR);
            return NULL;
...
static Node* create_node(char *name, UserType type, UserUnion data)
{
    User *userPTR = create_user(name, type, data);
    if (userPTR == NULL) {
        free(userPTR);
        return NULL;
    }
...
static int user_equal(const User *user1, const User *user2)
{
    ...
    if (user1->type == user2-> type && user1->type == STUDENT) {
        if (((user1->name == NULL && user2->name == NULL) || strcmp(user1->name, user2->name) == 0) && (student_equal(&((user1->data).student), &((user2->data).student)) == 1)) {
    	    return 1;
        }
    } else if(user1->type == user2-> type && user1->type == INSTRUCTOR) {
        if (((user1->name == NULL && user2->name == NULL) || strcmp(user1->name, user2->name) == 0) && (user1->data).instructor.salary == (user2->data).instructor.salary) {
    	    return 1;
        }
    }
...
{
    LinkedList *linkPTR = malloc(sizeof(LinkedList));
    if (linkPTR == NULL) {
        free(linkPTR);
        return NULL;
    }
...
int pop_front(LinkedList *list, User **dataOut)
{
    ...
    Node *returnNode = (list->head);
    (list->head) = ((list->head)->next);
    
    (*dataOut) = (returnNode->data);
    (list->size)--;
    free(returnNode);
    return 0;
}
...
int num_passing_all_classes(LinkedList *list, int *dataOut)
{
    ...
    for (Node *curr = list->head; curr != NULL; curr = curr->next) {
        if (curr->data->type == STUDENT) {
            int i = 0;
            int classesPass = 0;
                while (i < ((curr->data)->data).student.num_classes) {
                ...
                if (classesPass == ((curr->data)->data).student.num_classes) {
                    count++;
                }
...
int get_average_salary(LinkedList *list, double *dataOut)
{
    ...
    double totalSal = 0;
    if(list->size == 1) {
        if (((list->head)->data)->type == INSTRUCTOR) {
            (*dataOut) = ((list->head)->data->data).instructor.salary;
            return 0;
        } else {
            (*dataOut) = 0;
            return 0;
    ...
    for (Node *curr = list->head; curr != NULL; curr = curr->next) {
        if (curr->data->type == INSTRUCTOR) {
            totalSal += ((curr->data)->data).instructor.salary;
            count++;
        }
