...
public class Bases
{
    ...
    public static int binaryStringToInt(String binary)
    {
        ...
        for (int i = binary.length() - 1; i >= 0; i--) {
            total += (binary.charAt(i) - 48) << (binary.length() - 1 - i);
        }
    ...
    public static int hexStringToInt(String hex) {
        ...
        for (int i = hex.length() - 1; i >= 0; i--) {
            if (hex.charAt(i) >= 65) {
                total += (hex.charAt(i) - 55) << counter;
            } else {
                total += (hex.charAt(i) - 48) << counter;
            }
            counter += 4;
        }
        return total;
    ...
    public static String intToOctalString(int octal)
    {
        ...
        while (octal != 0) {
            result = ((char) ((octal & (0b111)) + 48)) + result;
            octal = (octal >> 3);
        }
        return result;
    ...
    public static String binaryStringToHexString(String binary)
    {
        ...
        for (int i = 31; i > 0; i-=4) {
            int result = ((binary.charAt(i) - 48) << 0) + ((binary.charAt(i-1) - 48) << 1) +
                    ((binary.charAt(i-2) - 48) << 2) + ((binary.charAt(i-3) - 48) << 3);
            total = (char) (result > 9 ? result + 55: result + 48) + total;
