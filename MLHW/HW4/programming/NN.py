...
class NeuralNet:
    ...
    def softmax(self, u):
        ...
        U_exponent = np.exp(u - U_max)

        total = (np.sum(U_exponent, axis=1, keepdims=True))
        o = U_exponent / total
    ...
    def _dropout(u, prob):
        ...
        dropout_mask = np.random.choice(range, u.shape, p=[prob, 1-prob])
    ...
    def cross_entropy_loss(self, y, y_hat):
        ...
        """
        log_y_hat = np.log(y_hat + (1e-15))

        loss = -1 * np.sum(y * log_y_hat)
        loss /= y.shape[0]
    ...
    def forward(self, x, use_dropout):
        ...
        u1 = np.matmul(x, self.parameters["theta1"]) + self.parameters["b1"] 
        ...
        u2 = np.matmul(o1, self.parameters["theta2"]) + self.parameters["b2"]
    ...
    def update_weights(self, dLoss, use_momentum):
        ...
        for l in dLoss:
            if use_momentum:
                self.change[l] *= self.momentum 
                self.change[l] +=  + dLoss[l]
                self.parameters[l] = self.parameters[l] - (self.learning_rate * self.change[l])
            else:
                self.parameters[l] = self.parameters[l] - (self.learning_rate * dLoss[l])
    ...
    def compute_gradients(self, y, yh, use_dropout):
        ...
        dLoss_u2 = (yh - y) / y.shape[0] # IMPLEMENT THIS LINE
        dLoss_theta2 = np.matmul(self.cache["o1"].T, dLoss_u2) # IMPLEMENT THIS LINE
        dLoss_b2 = np.sum(dLoss_u2, axis=0) # IMPLEMENT THIS LINE
        dLoss_o1 = np.matmul(dLoss_u2, self.parameters["theta2"].T) # IMPLEMENT THIS LINE

        if use_dropout:
            dLoss_u1 = dLoss_o1 * (self.cache["mask"] / (1 - self.dropout_prob)) * self.derivative_leaky_relu(self.alpha, self.cache["u1"])
        
        else:

            dLoss_u1 = dLoss_o1 * self.derivative_leaky_relu(self.alpha, self.cache["u1"])
        
        dLoss_theta1 = np.matmul(self.cache["X"].T, dLoss_u1)
    ...
    def gradient_descent(self, x, y, iter=60000, use_momentum=False, local_test=False):
        ...
        **For LOCAL TEST append and print out loss at every iteration instead of every 1000th multiple.
        ...
        for i in range(iter):
            ...
            loss = np.mean(self.cross_entropy_loss(y, y_hat))
