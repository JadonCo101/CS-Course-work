...
class RandomForest(object):
    ...
    def _bootstrapping(self, num_training, num_features, random_seed = None):
        ...
        row_idx = np.random.choice(np.arange(num_training), num_training, replace=True)
        #################################################################################
    ...
    def fit(self, X, y):
        ...
        for num in range(self.n_estimators):
            row_idx = self.bootstraps_row_indices[num]
            col_idx = self.feature_indices[num]
            X_train = X[:, col_idx]
            X_train = X_train[row_idx, :]
            y_train = y[row_idx]
            self.decision_trees[num].fit(X_train, y_train)
