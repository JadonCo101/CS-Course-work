...
class LogisticRegression(object):
    ...
    def predict_probs(
        ...
        h_x = 1 / (1 + np.exp(-(np.matmul(x_aug, theta))))
    ...
    def loss(self, y: np.ndarray, h_x: np.ndarray) -> float:  # [3pts]
        ...
        formula = -1 * ((y * (np.log(h_x))) + ((1 - y) * (np.log((1 - (h_x))))))
        loss =  float(np.sum(formula, axis=0) / (y.shape[0]))
    ...
    def gradient(
        ...
        error = (h_x - y)
        gradient = (1 / (y.shape[0])) * (np.matmul(x_aug.T, error))
    ...
    def fit(
        ...
        #2) augment data
        x_train_augment = self.bias_augment(x_train)
        
        #3) each epoch, a-d
        for epoch in range(epochs):
            
            h_x = self.predict_probs(x_train_augment, theta)
            
            gradient = self.gradient(x_train_augment, y_train, h_x)
