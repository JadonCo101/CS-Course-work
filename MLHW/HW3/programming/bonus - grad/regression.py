...
class Regression(object):
    ...
    def construct_polynomial_feats(
        ...
        if x.ndim == 1:
            ...
            feat = np.zeros((x.shape[0], degree+1, x.shape[1]))
    ...
    def linear_fit_GD(
        ...
        for i in range(epochs):
            ...
            y_predicted = np.matmul(xtrain, weight)
            temp = xtrain * (ytrain - y_predicted)
            calc = (np.sum(temp, axis=0) * (learning_rate / (len(xtrain))))[:, np.newaxis]
            
            
            weight += calc
                    
            y_predicted = np.matmul(xtrain, weight)
            loss_per_epoch.append(float(self.rmse(y_predicted, ytrain)))
    ...
    def linear_fit_SGD(
        ...
        for epoch in range(epochs):
            for p in range(xtrain.shape[0]):
                y_predicted = np.matmul((xtrain[p, :]), weight)
                calc = ((xtrain[p, :].T * (ytrain[p] - y_predicted)) * learning_rate)[:, np.newaxis]
                weight += calc
                y_predicted = np.matmul(xtrain, weight)
                loss_per_step.append(float(self.rmse(y_predicted, ytrain)))
    ...
    def ridge_fit_closed(
        ...
        bias_term = np.identity(xtrain.shape[1])
        bias_term[0,0] = 0
        
       
        weight = np.matmul((np.matmul((np.linalg.pinv((np.matmul(xtrain.T, xtrain)) + c_lambda * bias_term)), xtrain.T)), ytrain)
    ...
    def ridge_cross_validation(
        ...
        for i in range(kfold):
            ...
            y_pred = np.matmul(test_X, weight)
            rmse = self.rmse(test_y, y_pred)
