...
class ImgCompression(object):
    ...
    def rebuild_svd(
        ...
        if U_compressed.ndim == 2:
            X_rebuild = np.matmul(np.matmul(U_compressed, np.diag(S_compressed)), V_compressed)
            
        else:
            X = S_compressed[:, :, np.newaxis] * V_compressed
            X_rebuild = np.matmul(U_compressed, X)
    ...
    def compression_ratio(self, X: np.ndarray, k: int) -> float:  # [4pts]
        ...
        if X.ndim == 2:
            num_compressed_values = (X.shape[0] * k) + (k) + (k * X.shape[1])
            num_original_values = X.shape[0] * X.shape[1]
        
        else:
            num_compressed_values = (X.shape[1] * k) + (k) + (k * X.shape[2])
            num_original_values = X.shape[1] * X.shape[2]


        #(num stored values in compressed)/(num stored values in original)
    ...
    def recovered_variance_proportion(self, S: np.ndarray, k: int) -> float:  # [4pts]
        ...
        if S.ndim != 2:
            ...
            recovered_var = (np.sum(S_k, axis=1)) / (np.sum(S, axis=1))
