...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        raw = np.dot(X, self.weights['W1']) + self.weights['b1']

        sig = self.sigmoid(raw)

        second = np.dot(sig, self.weights['W2']) + self.weights['b2']

        soft = self.softmax(second)

        loss = self.cross_entropy_loss(soft, y)
        accuracy = self.compute_accuracy(soft, y)
        ...
        dL_dh2 = (soft - one_hot)
        dh2_dw2 = sig
        dL_dw2 = np.dot(dL_dh2.T, dh2_dw2).T

        #layer (2nd bias term)
        dL_db2 = np.mean(dL_dh2 * 1, axis=0)

        #layer with weights and sigmoid function
        dsig_dh1 = self.sigmoid_dev(raw)
        dh2_dsig = self.weights['W2']
        dh1_dw1 = X
        dL_dsig = np.dot(dL_dh2, dh2_dsig.T)
        dL_dh1 = (dL_dsig * dsig_dh1).T
        dL_dw1 = np.dot(dL_dh1, dh1_dw1).T

        #layer (1st bias term)
        dL_db1 = np.mean(dL_dh1.T * 1, axis=0)

        
        #grad second
        self.gradients['W2'] = dL_dw2 / N
        self.gradients['b2'] = dL_db2

        #grad of first
        self.gradients['W1'] = dL_dw1 / N
