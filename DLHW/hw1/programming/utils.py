...
def load_mnist_trainval():
    ...
    eighty_data = int((len(data)*0.8))
    eighty_lab = int((len(label)*0.8))
    train_data = data[0:eighty_data]
    val_data = data[eighty_data::]
    train_label = label[0:eighty_lab]
    val_label = label[eighty_lab::]
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    for idx in range(0, length, batch_size):
        batched_data.append(np.array(data[idx:min(idx + batch_size, length)]))
    
    total = len(label)
    for idx2 in range(0, total, batch_size):
        batched_label.append(np.array(label[idx2:min(idx2 + batch_size, total)]))
